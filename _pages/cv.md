---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

[one-page pdf version](https://qin-sihan.github.io/files/SihanCVmay26.pdf)

## 🎓 Education
**McGill University** | **CGPA: 3.9/4.0** (Exp. Grad '27)  
* Tomlinson Undergraduate Award (2024Fall, 2025 Winter, 2025 Fall)
* McGill French Immersion Award (2024 Summer, 2025 Summer)

---

## 🔬 Research Internship
### Study of LLM Bias on French Dialects | *Mar 2026 - Aug 2026* 
*Mitacs Accelerate - Armilla AI (Supervised by Prof. Jackie Cheung)* 
* Develop a taxonomy and evaluation metrics for dialect-based biases in LLMs
* Design benchmarks and preprocess datasets for French dialect evaluation
* Conduct API and local inference testing to analyze comparative bias metrics

---

## 🚀 Projects
### Visual Search App (CV & Training) | *Fall 2025* 
* Fine-tuned YOLOv8n for custom object detection
* Managed and annotated 100+ image dataset using Roboflow

### VPN-prototype (Cloud & OS) | *Summer 2025* 
*Second Prize @ NUS SOC Summer Workshop*
* Built and deployed a VPN service on AWS using Kubernetes for container orchestration


## 🏫 Teaching Experience
### Accessibility Tutor (Student Accessibility & Achievement)** | *Mar 2026 – Present*
  * Provide individualized 1-on-1 support to reinforce understanding of core course material for students with diverse learning needs.
### Undergraduate Student Course Assistant (USCA)** | *Jan 2025 – Apr 2026*
  * Graded homework and provided instructional support for **Linear Algebra (MATH 223)** and **Statistics (MATH 324)** courses, collaborating closely with fellow USCAs.
### TEAM Mentor in Computer Science** | *Sep 2024 – Dec 2025*
  * **Algorithms & Data Structures (COMP 251):** Assisted students with weekly exercises and managed academic inquiries on the Ed Discussion board.
  * **Intro to Computer Science (COMP 250):** Led small study groups of 15+ students and provided customized 1-on-1 support to improve programming fundamentals.


## 🏆 Extracurriculars
### McGill Students Chinese Brush Arts Club | *Sep 2023 – Present*
*President ← Co-President ← VP Media & Finance ← VP Art*

* **Leadership & Curriculum:** Oversee club operations and collaborate with the executive team to design and develop weekly art class curricula.
* **Event Coordination:** Plan and execute large-scale cultural events, such as the Spring Festival, including cross-club collaborations and logistical management.
* **Financial Management:** Manage the club budget, including operational costs, activity funding, and tuition collection.
* **Social Media & Engagement:** Lead digital outreach strategies, coordinating weekly updates and innovating content to enhance student engagement and community interaction.

### Hackathons & Competitions
* **Finalist:** BSRM x ALDO Retail Gen AI Hackathon (2024).
* **Participant:** MAIS Hacks (2025), NP-Complete-NAQ (2025), CodeJam14 (2024), Hack McWiCS (2024).


## 🛠 Technologies
* **Programming:** Python, Java, C/C++, Bash, SQL, OCaml, MIPS Assembly.
* **Tools:** PyTorch, NumPy, Git, Docker, Kubernetes, AWS, Conda.
* **Languages:** English (Fluent), Mandarin (Native), French (B1 in progress).